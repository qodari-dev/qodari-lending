...
import { useGenerateThirdPartyBalanceCertificateReport } from '@/hooks/queries/use-portfolio-report-queries';
import {
  GenerateThirdPartyBalanceCertificateBodySchema,
  GenerateThirdPartyBalanceCertificateReportResult,
...
import { z } from 'zod';
import { downloadPdfFromBase64 } from '@/app/credit-reports/components/pdf-download';

const FormSchema = GenerateThirdPartyBalanceCertificateBodySchema;
type FormValues = z.infer<typeof FormSchema>;

export function ThirdPartyBalanceCertificateReport() {
  const [result, setResult] = React.useState<GenerateThirdPartyBalanceCertificateReportResult | null>(
    null
  );
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: { thirdPartyDocumentNumber: '', cutoffDate: today },
  });

  const { mutateAsync: generatePdf, isPending: isGenerating } =
    useGenerateThirdPartyBalanceCertificateReport();
  const thirdPartyDocumentNumberValue = useWatch({
    control: form.control,
    name: 'thirdPartyDocumentNumber',
  });
  ...
  return (
    <>
      <PageHeader
        ...
        description="Genere el PDF del certificado consolidado de saldo por documento y fecha de corte."
      />
      <PageContent>
        <Card>
          <CardHeader>
            <CardTitle>Parametros</CardTitle>
            <CardDescription>Digite el documento del tercero y la fecha de corte.</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              <FieldGroup className="grid gap-4 md:grid-cols-[1fr_1fr_auto]">
                <Field data-invalid={Boolean(form.formState.errors.thirdPartyDocumentNumber)}>
                  <FieldLabel htmlFor="thirdPartyDocumentNumber">Documento tercero</FieldLabel>
                  <Input
