...
import { useProcessAccountingInterfaceLateInterest } from '@/hooks/queries/use-accounting-interface-queries';
import {
  ProcessAccountingInterfaceLateInterestBodySchema,
  ProcessAccountingInterfaceLateInterestResult,
...
const FormSchema = ProcessAccountingInterfaceLateInterestBodySchema;
...
export function InterfaceAccountingLateInterest() {
  const [result, setResult] = React.useState<ProcessAccountingInterfaceLateInterestResult | null>(null);
  ...
  const { mutateAsync: processLateInterest, isPending: isProcessing } =
    useProcessAccountingInterfaceLateInterest();

  const onSubmit = async (values: FormValues) => {
    const response = await processLateInterest({ body: values });
    setResult(response.body);
    toast.success('Solicitud enviada al backend');
  };

  return (
    <>
      <PageHeader
        title="Interface contable - Interes mora"
        description="Defina rango de fechas y fecha de transaccion para generar la interface."
      />
      <PageContent>
        <Card>
          <CardHeader>
            <CardTitle>Parametros</CardTitle>
            <CardDescription>Capture los datos para procesar la interface de interes mora.</CardDescription>
          </CardHeader>
          <CardContent>
