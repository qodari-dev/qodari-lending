...
import { useGenerateSettledCreditsReport } from '@/hooks/queries/use-credit-report-queries';
import {
  GenerateSettledCreditsReportBodySchema,
  GenerateSettledCreditsReportResult,
...
const FormSchema = GenerateSettledCreditsReportBodySchema;
type FormValues = z.infer<typeof FormSchema>;

export function SettledCreditsReport() {
  const [result, setResult] = React.useState<GenerateSettledCreditsReportResult | null>(null);
  ...
  const { mutateAsync: generateReport, isPending: isGenerating } = useGenerateSettledCreditsReport();
  ...
  const onDownload = React.useCallback(async () => {
    if (!result) return;
    await exportToExcel<SettledCreditsReportRow>(
      {
        title: 'Reporte de creditos saldados',
        filename: `creditos-saldados-${result.startDate}-${result.endDate}`,
        columns: [
  ...
  return (
    <>
      <PageHeader
        title="Reporte creditos saldados"
        description="Genere Excel de creditos saldados por rango de fechas."
      />
      <PageContent>
        <Card>
          <CardHeader>
