...
type CreateCurrentInterestRunInput = {
  processDate: Date;
...
export async function createAndQueueCurrentInterestRun(input: CreateCurrentInterestRunInput) {
  ...
  const existingRun = await db.query.processRuns.findFirst({
    where: and(
      eq(processRuns.processType, 'INTEREST'),
  ...
  try {
    await enqueueCurrentInterestJob({
...
function toSummary(value: unknown): CurrentInterestRunSummary {
  const fallback: CurrentInterestRunSummary = {
    reviewedCredits: 0,
    accruedCredits: 0,
    failedCredits: 0,
  ...
  const summary = value as Partial<CurrentInterestRunSummary>;
...
export async function getCurrentInterestRunStatus(processRunId: number) {
  ...
  if (run.processType !== 'INTEREST') {
    throwHttpError({
      status: 400,
      message: 'La corrida solicitada no corresponde a interés corriente',
      code: 'BAD_REQUEST',
    });
  }
