...
import { useGenerateMovementVoucherReport } from '@/hooks/queries/use-credit-report-queries';
import {
  GenerateMovementVoucherReportBodySchema,
  GenerateMovementVoucherReportResult,
...
const FormSchema = GenerateMovementVoucherReportBodySchema;
type FormValues = z.infer<typeof FormSchema>;

export function MovementVoucherReport() {
  const [result, setResult] = React.useState<GenerateMovementVoucherReportResult | null>(null);
  ...
  const { mutateAsync: generateReport, isPending: isGenerating } = useGenerateMovementVoucherReport();
  ...
  const onDownload = React.useCallback(async () => {
    if (!result) return;
    await exportToExcel<MovementVoucherReportRow>(
      {
        title: 'Comprobante de movimientos',
        filename: `comprobante-movimientos-${result.startDate}-${result.endDate}`,
        columns: [
          { header: '# Credito', accessorKey: 'creditNumber', width: 18 },
          { header: 'Fecha movimiento', accessorKey: 'movementDate', width: 18 },
          { header: 'Comprobante', accessorKey: 'voucherNumber', width: 18 },
          { header: 'Tipo', accessorKey: 'movementType', width: 16 },
          { header: 'Valor', width: 18, getValue: (row) => formatCurrency(row.amount) },
        ],
      },
      result.rows
    );
  ...
  return (
    <>
      <PageHeader
        title="Comprobante de movimientos"
        description="Genere Excel de comprobantes por rango de fechas."
      />
      <PageContent>
        <Card>
          <CardHeader>
