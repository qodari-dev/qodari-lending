...
import type { CausationScopeType, CurrentInsuranceRunSummary } from './types';

type CreateCurrentInsuranceRunInput = {
  processDate: Date;
...
export async function createAndQueueCurrentInsuranceRun(input: CreateCurrentInsuranceRunInput) {
  ...
  const existingRun = await db.query.processRuns.findFirst({
    where: and(
      eq(processRuns.processType, 'INSURANCE'),
  ...
  try {
    await enqueueCurrentInsuranceJob({
...
function toSummary(value: unknown): CurrentInsuranceRunSummary {
  const fallback: CurrentInsuranceRunSummary = {
    reviewedCredits: 0,
    accruedCredits: 0,
    failedCredits: 0,
  ...
  const summary = value as Partial<CurrentInsuranceRunSummary>;
...
export async function getCurrentInsuranceRunStatus(processRunId: number) {
  ...
  if (run.processType !== 'INSURANCE') {
    throwHttpError({
      status: 400,
      message: 'La corrida solicitada no corresponde a seguro',
      code: 'BAD_REQUEST',
    });
  }
