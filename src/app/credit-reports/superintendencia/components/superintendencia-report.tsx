...
import { useGenerateSuperintendenciaReport } from '@/hooks/queries/use-credit-report-queries';
import {
  GenerateSuperintendenciaReportBodySchema,
  GenerateSuperintendenciaReportResult,
...
const FormSchema = GenerateSuperintendenciaReportBodySchema;
type FormValues = z.infer<typeof FormSchema>;

export function SuperintendenciaReport() {
  const [result, setResult] = React.useState<GenerateSuperintendenciaReportResult | null>(null);
  ...
  const { mutateAsync: generateReport, isPending: isGenerating } = useGenerateSuperintendenciaReport();
  ...
  const onDownload = React.useCallback(async () => {
    if (!result) return;
    await exportToExcel<SuperintendenciaReportRow>(
      {
        title: 'Reporte superintendencia',
        filename: `superintendencia-${result.startDate}-${result.endDate}`,
        columns: [
          ...
          { header: 'Codigo reporte', accessorKey: 'reportCode', width: 18 },
        ],
      },
      result.rows
    );
  ...
  return (
    <>
      <PageHeader
        title="Reporte superintendencia"
        description="Genere Excel para reporte de superintendencia por rango de fechas."
      />
      <PageContent>
        <Card>
          <CardHeader>
