...
import { ProcessCausationCurrentInterestBodySchema } from '@/schemas/causation';
...
const scopeLabels: Record<FormValues['scopeType'], string> = {
  GENERAL: 'General',
  CREDIT_PRODUCT: 'Línea de crédito',
  LOAN: 'Crédito',
};
...
export function CausationCurrentInterest() {
  ...
  const { mutateAsync: processCurrentInterest, isPending: isProcessing } =
    useProcessCausationCurrentInterest();
  const { data: runStatusData, isFetching: isFetchingRunStatus } = useCurrentInterestRunStatus(runId);
  ...
  const onSubmit = async (values: FormValues) => {
    const response = await processCurrentInterest({
      body: {
        ...values,
  ...
  return (
    <>
      <PageHeader
        title="Causación - Interés corriente"
        description="Defina alcance, período y fecha de movimiento para ejecutar la causación."
      />
      <PageContent>
        <Card>
          <CardHeader>
            <CardTitle>Parámetros</CardTitle>
            <CardDescription>Capture los datos para procesar causación de interés corriente.</CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <FieldGroup className="grid gap-4 md:grid-cols-3">
                ...
                {scopeType === 'CREDIT_PRODUCT' ? (
                  <Controller
                    ...
                    render={({ field, fieldState }) => (
                      <Field data-invalid={fieldState.invalid}>
                        <FieldLabel htmlFor="creditProductId">Línea de crédito</FieldLabel>
                        <Select
                ...
                {scopeType === 'LOAN' ? (
                  <Controller
                    ...
                    render={({ field, fieldState }) => (
                      <Field data-invalid={fieldState.invalid}>
                        <FieldLabel htmlFor="loanId">Crédito</FieldLabel>
                        <Select
                          value={field.value ? String(field.value) : ''}
                          onValueChange={(value) => field.onChange(value ? Number(value) : undefined)}
